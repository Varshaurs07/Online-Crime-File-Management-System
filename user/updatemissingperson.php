...
{
    if (isset($_POST['updatemissingperson'])) 
    {
        $state = true;
 
        $uid = mysqli_real_escape_string($link, trim($_POST['updatemissingperson'])); 
        ...
        // $uid = "uid_".substr(bin2hex(random_bytes(10)),0, 10);
            ...
            $query = "update crime_2023_missingperson set notifylocation = '$location', notifydate = '$date', notifytime = '$time', notifyuser = '$user', notifyusername = '$username', status = 'closed' where uid = '$uid'";
            ...
            $querywanted = "select * from crime_2023_missingperson where uid = '$uid'";
        ...
        while ($rowemail  = mysqli_fetch_array($resultemail)) 
        {
            ...
            $name = $rowemail["name"];
            $message = "You've received an notification about missing person $missingperson. alias $missingperson. from $username. <br>
            ...
            $mailer->isHTML(true);
            $mailer->Subject = 'MISSING PERSON NOTIFICATION';
            $mailer->Body  = $message;
            $result = $mailer->send();
        }
            ...
            {
                $_SESSION["save"] = "yes";
                echo "<script> location.replace('missingpersons.php') </script>";
            } 
            else 
            {
                $_SESSION["fail"] = "yes";
